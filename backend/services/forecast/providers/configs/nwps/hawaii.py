...
class NWPSMauiModel(NWPSModelConfig):
    ...
    # TODO: change to time-deltas
    # long_wait_time = delta between model run and 6.5 hours
    # short time polls to make sure data is accessible, either by trying to access the
    # https url or not mabye via the status_file?

    model_long_wait_time: timedelta = time(6, 30, tzinfo=timezone.utc)
    model_short_wait_time: timedelta = time(0, 10, tzinfo=timezone.utc)
