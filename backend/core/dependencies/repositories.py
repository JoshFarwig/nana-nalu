...
def get_condition_profile_repository(
    session: AsyncSession = Depends(get_async_db_session),
) -> AsyncConditionProfileRepository:
    return AsyncConditionProfileRepository(session)


def get_crew_repository(
    session: AsyncSession = Depends(get_async_db_session),
    settings: APISettings = Depends(get_settings),
) -> AsyncCrewRepository:
    return AsyncCrewRepository(session, settings)
